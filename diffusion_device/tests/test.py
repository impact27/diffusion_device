...
class TestImage(TestCase):
    ...
    def test_bright(self):
        ...
        metadatafn = os.path.join(folder,
                                  'test_data/Brightim900ulph_Metadata.json')
        radius, profiles, fits, lse, pixel_size = full_fit(
            settingsfn, metadatafn)
    ...
    def test_film(self):
        ...
        metadatafn = os.path.join(folder,
                                  'test_data/327.68ul-h-50um device_Metadata.json')
        (radii, profiles_list,
    ...
    def test_12pos(self):
        ...
        metadatafn = os.path.join(folder,
                                  'test_data/350ulh_12pos/metadata.json')
        radius, profiles, fits, lse, pixel_size = full_fit(
            settingsfn, metadatafn)
        self.assertGreater(radius, 2.5e-9)
    ...
    def test_bright_walls(self):
        ...
        metadatafn = os.path.join(folder,
                                  'test_data/1tf_001tweentico75_100ulhr_Metadata.json')
        radius, profiles, fits, lse, pixel_size = full_fit(
            settingsfn, metadatafn)
        self.assertGreater(radius, 2e-9)
